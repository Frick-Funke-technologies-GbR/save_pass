...
class Backup {
  ...
  Future<bool> backupData() async { // validate this function
    Directory path = await getPwd();
    DateTime now = DateTime.now();
    CacheHandler cache = CacheHandler();
    DatabaseHandler db = DatabaseHandler();
    Directory backupsPath = Directory(path.path + '/backups'); // FIXME: Could be an other path in IOS
    backupsPath.createSync(); // create backups dir if not existing yet
    File backupFile = File(backupsPath.path + '/${now.toIso8601String().split('.')[0]}.spb');
